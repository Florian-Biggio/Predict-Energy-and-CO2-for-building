...
# Define the path to the encoder folder based on the current script location
service_dir = os.path.dirname(os.path.abspath(__file__))  # Get the absolute path of the script

model_ref = bentoml.xgboost.get("xgb_model_with_features:latest")


encoder_folder = os.path.abspath(os.path.join(os.getcwd(), "encoder"))  

encoder_path = os.path.join(encoder_folder, "one_hot_encoder.pkl")
scaler_path = os.path.join(encoder_folder, "std_scaler.pkl")
